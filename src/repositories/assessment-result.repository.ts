...
import { EmployeeAssessmentResult, AssessmentAnswer } from '../domain';

export class AssessmentResultRepository {
  async findById(resultId: string, tenantId: string): Promise<EmployeeAssessmentResult | null> {
    const result = await db.query(
      'SELECT * FROM employee_assessment_results WHERE id = $1 AND tenant_id = $2',
      [resultId, tenantId]
    );
    return result.rows.length > 0 ? this.mapToAssessmentResult(result.rows[0]) : null;
  }

  async findByEmployee(employeeId: string, tenantId: string): Promise<EmployeeAssessmentResult[]> {
    const result = await db.query(
      'SELECT * FROM employee_assessment_results WHERE employee_id = $1 AND tenant_id = $2 ORDER BY created_at DESC',
      [employeeId, tenantId]
    );
    return result.rows.map((row) => this.mapToAssessmentResult(row));
  }

  async findByAssessment(assessmentId: string): Promise<EmployeeAssessmentResult[]> {
    const result = await db.query(
      'SELECT * FROM employee_assessment_results WHERE assessment_id = $1 ORDER BY created_at DESC',
      [assessmentId]
    );
    return result.rows.map((row) => this.mapToAssessmentResult(row));
  }

  async findInProgress(employeeId: string): Promise<EmployeeAssessmentResult | null> {
    const result = await db.query(
      'SELECT * FROM employee_assessment_results WHERE employee_id = $1 AND status = $2 LIMIT 1',
      [employeeId, 'in-progress']
    );
    return result.rows.length > 0 ? this.mapToAssessmentResult(result.rows[0]) : null;
  }

  async findCompletedBySkill(employeeId: string, skillId: string): Promise<EmployeeAssessmentResult[]> {
    const result = await db.query(
      `SELECT ear.* FROM employee_assessment_results ear
       JOIN assessments a ON ear.assessment_id = a.id
       WHERE ear.employee_id = $1 AND a.skill_id = $2 AND ear.status = $3
       ORDER BY ear.completed_at DESC`,
      [employeeId, skillId, 'completed']
    );
    return result.rows.map((row) => this.mapToAssessmentResult(row));
  }

  async create(result: Omit<EmployeeAssessmentResult, 'id' | 'createdAt' | 'updatedAt'>): Promise<EmployeeAssessmentResult> {
    const dbResult = await db.query(
      `INSERT INTO employee_assessment_results
  ...
  async update(resultId: string, tenantId: string, updates: Partial<EmployeeAssessmentResult>): Promise<EmployeeAssessmentResult> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;
    ...
    const result = await db.query(
      `UPDATE employee_assessment_results SET ${fields.join(', ')}
  ...
  async getAggregateStats(skillId: string, tenantId: string): Promise<any> {
    const result = await db.query(
      `SELECT
       ...
       JOIN assessments a ON ear.assessment_id = a.id
       WHERE a.skill_id = $1 AND ear.tenant_id = $2`,
      [skillId, tenantId]
    );
  ...
  private mapToAssessmentResult(row: any): EmployeeAssessmentResult {
    return {
      id: row.id,
      ...
      answers: JSON.parse(row.answers || '[]') as AssessmentAnswer[],
