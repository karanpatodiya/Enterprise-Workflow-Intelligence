...
class LearningPathService {
    constructor(ragOrchestration, llmProvider) {
        this.ragOrchestration = ragOrchestration;
        this.llmProvider = llmProvider;
    }
    async generateLearningPath(req) {
        try {
            logger_1.logger.info('Generating learning path', { employeeId: req.employeeId, currentRole: req.currentRole });
            ...
            try {
                const ragContext = await this.ragOrchestration.executeRAGQuery({
                    ...
                    filters: { roleLevel: req.currentRole },
                });
                ragAnswer = ragContext.answer;
            }
            catch (error) {
                logger_1.logger.warn('RAG unavailable, using fallback curriculum context', error);
            }
            const targetedWeaknesses = req.weaknesses?.length ? req.weaknesses : ['General mastery'];
            let modulesData;
            try {
                const roadmap = await ai_service_1.AIService.generateLearningPath({
                ...
                logger_1.logger.warn('AI learning path generation failed, generating fallback curriculum', error);
            ...
            logger_1.logger.error('Failed to generate learning path', error);
            throw error;
        }
    }
    async saveCurriculumToDatabase(req, modulesData, report) {
        const client = await database_1.db.getClient();
        try {
            await client.query('BEGIN');
            // Create Path
            const pathRes = await client.query(`INSERT INTO learning_paths (
           employee_id,
           tenant_id,
           target_role,
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id`, [
            ...
            for (let i = 0; i < modulesData.length; i++) {
                ...
                const modRes = await client.query(`INSERT INTO learning_modules (path_id, title, content_markdown, order_index)
           VALUES ($1, $2, $3, $4) RETURNING id`, [pathId, mod.title, mod.content_markdown, i]);
                const moduleId = modRes.rows[0].id;
                if (mod.scenario) {
                    await client.query(`INSERT INTO scenario_exercises (module_id, scenario_text, expected_rubric, exercise_type, mcq_options, mcq_correct_option)
             VALUES ($1, $2, $3, $4, $5, $6)`, [
                        moduleId,
    ...
    async getActiveLearningPath(employeeId, tenantId) {
        const pathRes = await database_1.db.query(`SELECT * FROM learning_paths WHERE employee_id = $1 AND tenant_id = $2 AND status = 'in_progress' ORDER BY created_at DESC LIMIT 1`, [employeeId, tenantId]);
        if (pathRes.rows.length === 0)
            return null;
        const path = pathRes.rows[0];
        const modulesRes = await database_1.db.query(`SELECT m.*, 
        json_build_object(
          'id', s.id, 
       ...
       WHERE m.path_id = $1
       ORDER BY m.order_index ASC`, [path.id]);
        return {
            ...path,
            modules: modulesRes.rows
        };
    }
    async submitScenarioAnswer(exerciseId, userAnswer) {
        // 1. Fetch scenario and rubric
        const exerciseRes = await database_1.db.query(`SELECT e.*, m.title FROM scenario_exercises e JOIN learning_modules m ON e.module_id = m.id WHERE e.id = $1`, [exerciseId]);
        ...
        if (exercise.exercise_type === 'mcq') {
            ...
            try {
                const result = await ai_service_1.AIService.evaluateAnswer(exercise.scenario_text, userAnswer, exercise.expected_rubric || '');
                grading = {
        ...
        // 3. Update database
        await database_1.db.query(`UPDATE scenario_exercises SET user_answer = $1, grade = $2, ai_feedback = $3 WHERE id = $4`, [userAnswer, grading.grade, grading.feedback, exerciseId]);
        // If passed, mark module complete
        if (grading.grade === 'pass') {
            await database_1.db.query(`UPDATE learning_modules SET status = 'completed' WHERE id = $1`, [exercise.module_id]);
        }
        return grading;
    }
    buildFallbackCurriculum(req, ragAnswer) {
        ...
        return topics.map((topic, index) => {
            ...
            return {
                ...
                scenario: isMCQ ? {
                    ...
                    exercise_type: 'scenario',
                    scenario_text: `You are leading a high-impact initiative related to "${topic}". Describe your approach, key decisions, risk handling, and success criteria.`,
                    expected_rubric: 'Clear problem framing; prioritized plan; explicit tradeoffs; risk mitigation; measurable outcomes',
                },
                order_index: index,
            };
        });
    }
    fallbackGrade(expectedRubric, userAnswer) {
        ...
        return {
            ai_detected: false,
            grade: 'fail',
            feedback: 'Please provide a bit more detail. Include your plan, key tradeoffs, risks, and at least one measurable outcome.',
        };
    }
}
